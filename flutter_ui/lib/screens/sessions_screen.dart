...
class _SessionScreenState extends State<SessionScreen>
  ...
  void _showCreateSessionDialog() {
    ...
    _endDateController.text =
        DateFormat('yyyy-MM-dd HH:mm').format(selectedEndDate);
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildTextField(_dateController, 'Start Date & Time',
                    onTap: () async {
                ...
                SizedBox(height: 16),
                _buildTextField(_endDateController, 'End Date & Time',
                    onTap: () async {
                  final DateTime? pickedDate = await showDatePicker(
                    context: context,
                ...
                SizedBox(height: 16),
                _buildTextField(_commentController, 'Comment (Optional)',
                    maxLines: 3),
              ],
            ),
          ),
  ...
  Widget _buildTextField(TextEditingController controller, String label,
      {VoidCallback? onTap, bool readOnly = false, int maxLines = 1}) {
    return Padding(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(context);
              try {
                await ApiService.deleteSession(sessionId, widget.groupId);
                _refreshSessions();
                ScaffoldMessenger.of(context).showSnackBar(
              ...
              onPressed: () async {
                try {
                  ...
                  await ApiService.updateSession(
                      widget.groupId, sessionId, sessionData);
                  _refreshSessions();
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
